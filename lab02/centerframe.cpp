...
void CenterFrame::createUserCommandArea()
{
    ...
    p.fill(BACKGROUND_COLOR);
    //菱形的四个顶点
    QPointF pn1(3,p.size().height()/2);
    QPointF pn2(p.size().width()/2,3);
    QPointF pn3(p.size().width()-3,p.size().height()/2);
    QPointF pn4(p.size().width()/2,p.size().height()-3);
    QVector<QPointF> pns;
    pns.clear();
    pns<<pn1<<pn2<<pn2<<pn3<<pn3<<pn4<<pn4<<pn1;

    painter.drawPolygon(pns);
